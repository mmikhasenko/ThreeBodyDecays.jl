...
@testset "phase space mapping" begin
    ...
    f = phase_space_integrand_ki(σs -> 1, ms; k=2)
...
@testset "three-body phase-space integral value" begin
    ...
    # 
    integrand = phase_space_integrand_ki(σs -> 1, ms; k=1)
    value_1 = cuhre((x, f)->(f[1] = integrand(x)), 2, 1)[1][1]
    integrand = phase_space_integrand_ki(σs -> 1, ms; k=2)
    value_2 = cuhre((x, f)->(f[1] = integrand(x)), 2, 1)[1][1]
    integrand = phase_space_integrand_ki(σs -> 1, ms; k=3)
