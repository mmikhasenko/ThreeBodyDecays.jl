...
@testset "Hermitian structure" begin
    ...
    @test maximum(abs(imag(chain_physical.matrix[i, i])) for i in eachindex(chain_physical.labels)) ≤ 1e-10
    @test all(real(chain_physical.matrix[i, i]) ≥ -1e-10 for i in eachindex(chain_physical.labels))
end

@testset "Event-level overlaps reduce correctly" begin
    mean_chain = mean(event_chain_overlaps)
    @test Matrix(mean_chain.matrix) ≈ Matrix(chain_stripped.matrix) ./ length(event_chain_overlaps) atol = 1e-10 rtol = 1e-10
...
@testset "Quadratic form reproduces direct intensities" begin
    ...
    @test real(model.couplings' * Matrix(chain_stripped.matrix) * model.couplings) ≈ direct_total atol = 1e-9 rtol = 1e-10

    random_couplings = complex.(randn(length(model.couplings)), randn(length(model.couplings)))
    random_model = ThreeBodyDecay(collect(model.chains), random_couplings, collect(model.names))
    random_total = sum(σs -> unpolarized_intensity(random_model, σs), samples)
    @test real(random_couplings' * Matrix(chain_stripped.matrix) * random_couplings) ≈ random_total atol = 1e-8 rtol = 1e-10
end
...
@testset "Fit fractions and pair interferences match direct grouped sums" begin
    ...
    direct_diagonal = Dict(
        label => sum(σs -> unpolarized_intensity(submodel, σs), samples)
        for (label, submodel) in zip(labels, submodels)
    )
    direct_pairs = Dict{Tuple{String,String},Float64}()
    for j in 2:length(labels), i in 1:(j - 1)
    ...
    fractions = Dict(row.label => row.fraction for row in fit_fractions(name_physical; percent = false, sort = false))
    for label in labels
        @test fractions[label] ≈ direct_diagonal[label] / total_intensity(name_physical) atol = 1e-10 rtol = 1e-10
    end

    pair_rows = Dict((row.label_i, row.label_j) => row.fraction for row in interference_terms(name_physical; percent = false, sort = false))
    for (pair, value) in direct_pairs
        @test pair_rows[pair] ≈ value / total_intensity(name_physical) atol = 1e-9 rtol = 1e-10
    end
end

@testset "Vector APIs aggregate before normalization" begin
    event_name_physical =
        group_overlap.(physical_overlap.(event_chain_overlaps, Ref(model.couplings)), Ref(model.names))

    fractions_from_vector = Dict(row.label => row.fraction for row in fit_fractions(event_name_physical; percent = false, sort = false))
    fractions_from_mean = Dict(row.label => row.fraction for row in fit_fractions(mean(event_name_physical); percent = false, sort = false))
    @test fractions_from_vector == fractions_from_mean

    interferences_from_vector =
        Dict((row.label_i, row.label_j) => row.fraction for row in interference_terms(event_name_physical; percent = false, sort = false))
    interferences_from_mean =
        Dict((row.label_i, row.label_j) => row.fraction for row in interference_terms(mean(event_name_physical); percent = false, sort = false))
